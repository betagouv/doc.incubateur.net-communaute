...
* Vous recevrez une invitation par e-mail de la part de YesWeHack à créer un
  compte sur leur plateforme pour pouvoir accéder de manière sécurisée au
  rapport de vulnérabilité.
* Vous serez aussi invité sur un canal Mattermost privé nommé selon
...
Non, vous recevrez des communications uniquement si une vulnérabilité avérée a
été identifiée sur le périmètre de votre start-up d'État.

### Je dois faire quelque chose de particulier ?

Vérifiez juste que la liste des membres de votre SE est à jour sur votre fiche
produit, c'est cette liste qui est utilisée pour vous contacter en cas de
rapport de vulnérabilité.
...
### Du coup, ma start-up est protégée des vraies attaques ?

Non, le programme de Bug Bounty est une mesure de prévention **parmi** d'autres.
Vous devez continuer à vous assurer que votre service est bien sécurisé.

### Puis-je, en tant que (ex) membre de la communauté beta.gouv.fr, participer au Bug Bounty ?

Non, du moins vous ne pourrez pas être rémunéré au même titre que les hackeurs
éthiques.
Dans le cas ou vous identifieriez une vulnérabilité, vous pouvez directement
